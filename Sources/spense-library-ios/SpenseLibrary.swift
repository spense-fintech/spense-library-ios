...
public class SpenseLibrary {
    ...
    func checkLogin() async throws -> [String: Any] {
    ...
    func login(token: String) async throws -> [String: Any] {
        guard let hostName = self.hostName else {
            throw SpenseError.hostnameNotSet
        }
    ...
    func open(on viewController: UIViewController, withSlug slug: String) {
